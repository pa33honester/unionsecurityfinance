...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['firstname'])) {
  ...
  @file_put_contents('/tmp/open_user_account.log', date('c') . " POST:\n" . print_r($_POST, true) . "\n", FILE_APPEND);
  // assume $conn exists from header.php
  $firstname = $_POST['firstname'] ?? '';
  $middlename = $_POST['middlename'] ?? '';
  $lastname = $_POST['lastname'] ?? '';
  $country = $_POST['country'] ?? ($_POST['countryId'] ?? '');
  $state = $_POST['state'] ?? ($_POST['stateId'] ?? '');
  $city = $_POST['city'] ?? ($_POST['cityId'] ?? '');
  ...
  $accountbalance = $_POST['accountbalance'] ?? '0';
  $imf = $_POST['imf'] ?? '';
  $cot = $_POST['cot'] ?? '';
  ...
  $sql = "INSERT INTO users (
      firstname, middlename, lastname, country, state, city, zipcode, dob, address, phone, email,
      next_of_kin, next_of_kin_phone, next_relationship,
      accountnumber, accounttype, usercurrency, accountbalance, imf, cot, secretCode, status, password
    ) VALUES (
      '$firstname','$middlename','$lastname','$country','$state','$city','$zipcode','$dob','$address','$phone','$email',
      '$next_of_kin','$next_of_kin_phone','$next_of_kin_relationship',
      '$accountnumber','$accounttype','$usercurrency','$accountbalance','$imf','$cot','$secretCode','active','$password'
    )";
        ...
        <div class="row">
          ...
          <div class="col-md-6">
            <div class="form-group">
              ...
              <input type="text" name="next_of_kin_phone" class="form-control" placeholder="Enter Phone Number">
            </div>
          </div>
          ...
          <div class="col-md-6">
            <div class="form-group">
              ...
              <input type="text" name="next_of_kin_relationship" class="form-control" placeholder="Relationship e.g. Brother">
            </div>
          </div>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      $('.regUser').click(function(e) {
        ...
        var next_of_kin_relationship = $('input[name="next_of_kin_relationship"]').val();
        ...
        $.ajax({
          ...
          data: {
            ...
            "next_of_kin_phone": $('input[name="next_of_kin_phone"]').val(),
          ...
          success: function(data) {
            $('.regResult').html(data);
            try {
              // Execute any inline <script> blocks returned by the server
              var scripts = $('<div>').html(data).find('script');
              scripts.each(function() { $.globalEval($(this).text()); });
            } catch (e) {}
            try {
              $('#form')[0].reset();
            } catch (e) {}
          ...
          error: function(xhr, status, err) {
            var msg = 'Request failed: ' + xhr.status + ' ' + xhr.statusText + '\n' + (xhr.responseText || err);
            $('.regResult').html('<div class="alert alert-danger">' + msg + '</div>');
            console.error('open_user_account POST error:', status, err, xhr.responseText);
            // also append to debug log
            try {
              var log = 'JS_ERROR ' + new Date().toISOString() + ' ' + msg + '\n';
              // fire-and-forget fetch to a logging endpoint could be added; for now instruct user to check /tmp/open_user_account.log on server
            } catch (e) {}
          }
        });
      });
