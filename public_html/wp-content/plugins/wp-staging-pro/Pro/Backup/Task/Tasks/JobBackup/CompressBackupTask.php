...
class CompressBackupTask extends BackupTask
{
     ...
     *                      It's the same index file used by the Archiver.
     *
     * @see Archiver::$tempBackupIndex
     */
    ...
    public function execute(): TaskResponseDto
    {
        ...
        $this->tempBackupIndex = $this->archiver->getTempBackupIndex();
        $this->tempBackup      = $this->archiver->getTempBackup();
        ...
        $compressedIndexFileObject  = null;
        $compressedBackupFileObject = null;
        ...
        if ($this->stepsDto->isFinished()) {
            ...
            $originalTmpBackupName = $this->tempBackup->getFilename();
            // Extra step on Windows OS, renaming make sure the file is not locked.
            if ($this->getIsWindowsOs()) {
                $this->tempBackup->rename('wpstg_tmp_renamed_' . time());
            }

            $this->tempBackup->delete();
            $this->compressedBackup->rename($originalTmpBackupName);
    ...
    protected function getIsWindowsOs(): bool
    {
