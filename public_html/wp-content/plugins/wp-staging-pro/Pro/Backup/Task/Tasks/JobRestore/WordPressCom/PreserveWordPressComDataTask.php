...
class PreserveWordPressComDataTask extends RestoreTask
{
    ...
    public function execute()
    {
        ...
        if ($this->jobDataDto->getIsMissingDatabaseFile()) {
            $this->logger->warning('Skipped preserving WordPress.com data.');
    ...
    protected function addMasterUser(array $masterUser): int
    {
        ...
        $this->logger->info('Added current site master user');
        return $userID;
    }
    ...
    protected function maybePreserveJetpackData()
    {
        ...
        $this->logger->info('Preserved Jetpack data');
    }
    ...
    protected function maybeDisableJetpackModules()
    {
        ...
        if ($result) {
            $this->logger->info(sprintf('Disabled Jetpack modules: %s', esc_html(implode(', ', $modulesToDisable))));
        } else {
            $this->logger->warning(sprintf('Failed to disable Jetpack modules: %s', esc_html(implode(', ', $modulesToDisable))));
        }

        if ($isSSOModuleDisabled) {
            $this->logger->warning('SSO Module is disabled because current site master user has different ID than the same user in backup.');
        }
    }
    ...
    protected function maybeUpdateMasterUserInJetpackOptions()
    {
        ...
        if ($result) {
            $this->logger->info('Updated master user in Jetpack options');
        } else {
            $this->logger->warning('Failed to update master user in Jetpack options');
        }
        ...
        if ($result) {
            $this->logger->info('Updated master user token in Jetpack options');
        } else {
            $this->logger->warning('Failed to update master user token in Jetpack options');
        }
    }
}
