...
WP STAGING is a professional all in one <strong>backup, duplicator and staging plugin</strong>. Create an exact copy and backup of your website in seconds. Perfect for staging, backup, or development purposes.
...
= 5.8.3 =
* Fix: Make sure that `Show Logs` button should have consistent margins when rendering process logs. #3604
* Fix: Disable email notifications when a backup successfully runs. #3517
* Fix: Properly catch fatal errors when merging logs into single file when sending for error report. #3573
* Fix: Make the backup restorable even when it has not correctly replaced table Constraint(s). #3595
* Fix: Make sure to update new admin login password if account already exists while creating staging site.(Pro) #3598
* Fix: Refresh error message when clicking the backup tab. #3587
* Dev: Refactor remote storage downloading specific code to be separate classes. (pro) #2751
...
= 5.8.1 =
* Fix: WP Staging Restore failed to read the cache file with old object data when using PHP 7.2. #3539
* Fix: Make sure to display correct time stamp when retrieving remote backups from an FTP storage provider. (Pro) #3499
...
== Upgrade Notice ==
Compatible up to WordPress 6.7.2. New plugin auto update feature for staging sites.
